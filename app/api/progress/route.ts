...
const CACHE_FILE = join(os.tmpdir(), "afss-progress-cache.json");
...
async function fetchProgressCFSP(): Promise<Record<string, unknown>[]> {
  ...
  while (page <= 100) {
    ...
    const url =
      ...
      `?pageSize=250&columns=${LIST_COLS}&Stage=Progress&page=${page}`;
...
async function fetchAndCache(): Promise<Record<string, unknown>[]> {
  const cfspJobs = await fetchProgressCFSP();
