...
export default function BacklogPage() {
  ...
  return (
    <div className="flex flex-col h-screen" onClick={() => setDropdownOpen(false)}>
      ...
      <div className="flex items-center gap-0 bg-white border-b border-neutral-200 shrink-0">

        {/* Company dropdown */}
        <div className="relative" onClick={(e) => e.stopPropagation()}>
          <button
            onClick={() => setDropdownOpen((o) => !o)}
            className="flex items-center gap-2 px-4 py-3 text-sm font-semibold text-neutral-800 hover:bg-neutral-50 border-r border-neutral-200 min-w-[200px]"
          >
          ...
          {dropdownOpen && (
            <div className="absolute top-full left-0 z-50 bg-white border border-neutral-200 rounded-b shadow-lg min-w-[200px]">
              {COMPANIES.map((co) => (
                <button
                  key={co.id}
        ...
        <select
          ...
          className="px-5 py-3 text-sm font-medium text-neutral-500 hover:text-neutral-800 border-r border-neutral-200 hover:bg-neutral-50 bg-white cursor-pointer"
        >
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-white border-b border-neutral-200 text-xs text-neutral-500 shrink-0">
        <div className="flex items-center gap-3">
          <span className="font-semibold text-neutral-800 text-sm">
            {companyLabel} — {stage}{company === 8 ? "" : " · A CFSP ONLY"}
          </span>
          {!loading && (
            <span className={`px-2 py-0.5 rounded-full font-semibold text-xs ${stageColor}`}>
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-neutral-300">Auto every 60s</span>
        </div>
      </div>
